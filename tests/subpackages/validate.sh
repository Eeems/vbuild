...
if ! grep -q "qt_command_executor()" APKBUILD; then
  echo "subpackage function qt_command_executor missing"
  exit 1
fi
if ! grep -q "message_broker()" APKBUILD; then
  echo "subpackage function message_broker missing"
  exit 1
fi
if ! grep -q "subpackage()" APKBUILD; then
  echo "subpackage function subpackage missing"
  exit 1
fi
